...
func NewDeviceHealthUseCase(
	...
	return &useCaseImpl{
		...
		cooldownManager: NewCooldownManager(config.CooldownPeriod),
		deduplicator:    NewEventDeduplicator(config.DeduplicationWindow),
		config:          config,
		logger:          logger,
...
func (uc *useCaseImpl) ProcessDeviceDetectedEvent(ctx context.Context, event *entities.DeviceDetectedEvent) error {
	...
	if !uc.deduplicator.ShouldProcess(event) {
		uc.logger.Warn("Duplicate device detected event, skipping",
	...
	if !uc.cooldownManager.CanCheck(event.MACAddress) {
		timeUntilNext := uc.cooldownManager.GetTimeUntilNextCheck(event.MACAddress)
		uc.logger.Info("Device is in cooldown period, skipping health check",
			"mac_address", event.MACAddress,
			"time_until_next_check", timeUntilNext)
		return nil
	}
...
func (uc *useCaseImpl) performHealthCheck(ctx context.Context, event *entities.DeviceDetectedEvent) {
	...
	defer uc.cooldownManager.MarkChecked(event.MACAddress)
...
func (uc *useCaseImpl) performCleanup() {
	...
	uc.cooldownManager.Cleanup()
	uc.deduplicator.Cleanup()

	uc.logger.Debug("Health check cleanup completed",
		"tracked_events", uc.deduplicator.GetEventCount())
}
